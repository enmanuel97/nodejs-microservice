...
exports.recibirPedido = async (req, res) => {
	try {
		const receta = await axios.get(`http://localhost:3000/receta`);
...
exports.prepararPedido = async (req, res) => {
	try {
		...
		const receta = await axios.get(`http://localhost:3000/receta/${pedido.recetaId}`);
		...
		const {data: stock} = await axios.get(`http://localhost:3000/bodega/inventario/${ingredientsString.slice(0, -1)}`);
		...
		if(itemWithoutStock > 0) {
			...
			await axios.post(`http://localhost:3000/bodega/actualizar-inventario/retirar`, {
