...
module.exports = () => {
	...
	router.get('/cocina', async (req, res) => {
		...
		const { data: pedidos } = await axios.get(`http://localhost:4001/api/cocina/ultimos-pedidos/1,2`);
	...
	router.get('/cocina/historial-pedidos', async (req, res) => {
		const { data: pedidos } = await axios.get(`http://localhost:4001/api/cocina/ultimos-pedidos/3`);
